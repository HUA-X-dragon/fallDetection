...
def main(opt, file_name):
    ...
    for i in file_name:
        ...
        if len(pred_boxes) == 0:
            sub.append(i+' '+'stand'+' '+str(int(frame.shape[1]/5*2))+' '+str(int(frame.shape[0]/5*3))+' '+str(int(frame.shape[1]/5*2))+' '+str(int(frame.shape[0]/5*3)))
        else:
            for j, _ in enumerate(pred_boxes):
                box = pred_boxes[j]
                ...
                sub.append(i+' '+lt1[pred_classes[j]]+' '+str(x0)+' '+str(y0)+' '+str(x1)+' '+str(y1))
        count+=1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='/home/jovyan/best.onnx', help='onnx path(s)')
